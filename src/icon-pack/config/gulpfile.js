const gulp = require('gulp'),
  ...
  fs = require('fs');
const svg_files_folder = './_icons-svg',
  dest_folder = '../icons',
  ...
  return (
    gulp
        ...
        svgmin({
          ...
          plugins: [
            ...
            // "removeEmptyContainers",
            {
              name: 'removeAttrs',
              params: {
                attrs: ['(filter|style|class|data.*)', 'svg:(width|height)'],
              },
            },
            // "removeUselessDefs",
          ],
      ...
      .pipe(
        cheerio({
          run: function ($) {
            ['#17191c', '#292d32', '#4cbd00'].map(item => {
              $(`[fill="${item}"]`).attr('fill', 'currentColor');
              $(`[stroke="${item}"]`).attr('stroke', 'currentColor');
            });
            $('[stroke-width]').attr('stroke-width', '1.5');
          },
          parserOptions: { xmlMode: true },
        })
      )
      // Replace 'data-name' with 'dataName'
      .pipe(replace(/data-(\w)/g, (match, p1) => `data${p1.toUpperCase()}`))
      // Replace 'stroke-linecap' with 'strokeLinecap'
      .pipe(replace(/stroke-(\w)/g, (match, p1) => `stroke${p1.toUpperCase()}`))
      // Remove <svg> tags
      .pipe(replace(/<\/?svg(.*?)>/g, ''))
